...
def compute_entity_quality(
	...
	if fields:
		missing_fields = [
			...
			if not has_evidence_for_field(evidence_fields, field)
		]
	...
	if rule_set.get("evidence_quote_required", {}).get("enabled"):
		if has_empty_evidence_quote(evidence_fields):
			flags.append("evidence_missing_quote")

	if rule_set.get("both_peptide_and_molecule", {}).get("enabled"):
		if has_peptide_data(entity_row) and has_molecule_data(entity_row):
	...
	if temp_rule.get("enabled") and entity_row.temperature_c is not None:
		if entity_row.temperature_c < temp_rule.get("min", -50) or entity_row.temperature_c > temp_rule.get("max", 150):
			flags.append("invalid_temperature")

	if rule_set.get("concentration_nonnegative", {}).get("enabled") and entity_row.concentration is not None:
		if entity_row.concentration < 0:
...
def list_non_null_fields(entity_payload: Dict[str, Any]) -> List[str]:
	...
	for key in ["sequence_one_letter", "sequence_three_letter", "n_terminal_mod", "c_terminal_mod", "is_hydrogel"]:
