...
class QueueCoordinator:
    ...
    def enqueue_existing_run(
        ...
        *,
        ...
        try:
            session.add(run)
            self._release_locks_for_run_except(
                session,
                run_id=run.id,
                keep_fingerprints=fingerprints,
            )
            if existing_job:
                self._ensure_locks(
                    session,
    ...
    def _ensure_locks(
        ...
        *,
        ...
        existing = set(
            session.exec(
                select(ActiveSourceLock.source_fingerprint).where(
                    ActiveSourceLock.source_fingerprint.in_(fingerprints)
                )
            ).all()
        )
        now = utc_now()
    ...
    @staticmethod
    def _release_locks_for_run_except(
        session: Session,
        *,
        run_id: int,
        keep_fingerprints: list[str],
    ) -> None:
        stmt = delete(ActiveSourceLock).where(ActiveSourceLock.run_id == run_id)
        if keep_fingerprints:
            stmt = stmt.where(ActiveSourceLock.source_fingerprint.notin_(keep_fingerprints))
        session.exec(stmt)
